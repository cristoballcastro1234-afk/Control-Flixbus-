  ...
  const { results } = await context.env.DB
    .prepare(
      `SELECT fecha, destino, slot_index as slotIndex, hora_plan as horaPlan,
              bus_num as busNum, hora_llegada as horaLlegada, hora_real as horaReal, cancelado, updated_at as updatedAt
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  await context.env.DB
    .prepare(
      `INSERT INTO turnos (fecha, destino, slot_index, hora_plan, bus_num, hora_llegada, hora_real, cancelado, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    .bind(fecha, destino, slotIndex, horaPlan, busNum, horaLlegada, horaReal, cancelado ? 1 : 0, updatedAt)
    .run();

  return json({ fecha, destino, slotIndex, horaPlan, busNum, horaLlegada, horaReal, cancelado, updatedAt });
};
